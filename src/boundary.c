...
static unsigned long _index(cmaes_boundary_t * t, unsigned long i);
static void _FatalError(const char *s);
static double default_lower[1];
static double default_upper[1];

void cmaes_boundary_init(cmaes_boundary_t * t,
...
void cmaes_boundary_exit(cmaes_boundary_t * t)
{
	if (t->al)
		free(t->al);
	if (t->au)
...
void cmaes_boundary_transformation(cmaes_boundary_t * t, double const *x, double *y, unsigned long len)
{
	double lb, ub, al, au;
	unsigned long i;
	cmaes_boundary_shift_into_feasible_preimage(t, x, y, len);
	for (i = 0; i < len; ++i) {
...
void cmaes_boundary_shift_into_feasible_preimage(cmaes_boundary_t * t, double const *x,
																double *y, unsigned long len)
{
	...
	for (i = 0; i < len; ++i) {
		...
		if ((y[i] < lb - al - 1e-15) || (y[i] > ub + au + 1e-15)) {
			printf
				("BUG in cmaes_boundary_shift_into_feasible_preimage: lb=%f, ub=%f, al=%f au=%f, y=%f\n",
				 lb, ub, al, au, y[i]);
			_FatalError("BUG");
		}
	}
...
void cmaes_boundary_transformation_inverse(cmaes_boundary_t * t,
...
/*
static void _manage_len(cmaes_boundary_t *t, unsigned int len)
{
