 ...
 *
 * * cmaes_boundary_init(this, l_bound, u_bound, len)
 * * cmaes_boundary_exit(this)
...
void cmaes_boundary_init(cmaes_boundary_t *,
										double const *lower_bounds, double const *upper_bounds,
										unsigned long len_of_bounds);

/* release memory */
void cmaes_boundary_exit(cmaes_boundary_t *);
 ...
 * */
void cmaes_boundary_transformation(cmaes_boundary_t *, double const *x, double *y, unsigned long len);	/* new value into y */
 ...
 * */
void cmaes_boundary_transformation_inverse(cmaes_boundary_t * t, double const *y, double *x, unsigned long len);	/* new value into x */

/* used by function cmaes_boundary_transformation. After applying the shift,
 *   cmaes_boundary_shift_into_feasible_preimage(b,x,x,l)
 * the two consecutive calls
 *   cmaes_boundary_transformation(b,x,y,l) ; cmaes_boundary_transformation_inverse(b,y,x,l) ;
 * have no effect on x anymore */
void cmaes_boundary_shift_into_feasible_preimage(cmaes_boundary_t * t, double const *x, double *x_shifted, unsigned long len);	/* new value into x_shifted */
