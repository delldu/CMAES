...
#include "cmaes.h"				/* <time.h> via cmaes.h */
...
const double *cmaes_Optimize(cmaes_t *, double (*pFun) (double const *, int dim), long iterations);
					 ...
					 long int inseed, int lambda, const char *input_parameter_filename)
{
	...
	cmaes_readpara_init(&t->sp, dimension, inxstart, inrgstddev, inseed, lambda, input_parameter_filename);
}
...
double *cmaes_init(cmaes_t * t,	/* "this" */
				   ...
				   long int inseed, int lambda, const char *input_parameter_filename)
{
	cmaes_init_para(t, dimension, inxstart, inrgstddev, inseed, lambda, input_parameter_filename);
 ...
 */
{
	...
	while (key < keyend) {
		...
		if (strncmp(key, "few", 3) == 0) {
			...
			fprintf(fp, "  %.2e  %.2e %.2e", t->sigma, t->sigma * sqrt(t->maxdiagC), t->sigma * sqrt(t->mindiagC));
...
cmaes_readpara_init(cmaes_readpara_t * t,
					int dim,
					const double *inxstart, const double *inrgsigma, int inseed, int lambda, const char *filename)
{
	int i, N;
...
void cmaes_readpara_ReadFromFile(cmaes_readpara_t * t, const char *filename)
{
	...
	for (ipara = 0; ipara < t->n2para; ++ipara) {
		rewind(fp);
		while (fgets(s, sizeof(s), fp) != NULL) {	/* read one line *//* skip comments  */
			if (s[0] == '#' || s[0] == '%')
				continue;
...
void cmaes_readpara_WriteToFile(cmaes_readpara_t * t, const char *filenamedest)
{
	...
	fprintf(fp, "\n# Read from %s at %s\n", t->filename ? t->filename : "", asctime(localtime(&ti)));	/* == ctime() */
